...
var svg = d3.select('#chart-1').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').attr('transform', "translate(".concat(width / 2, ",").concat(height / 2, ")")); // At the very least you'll need scales, and
...
var labelArc = d3.arc().innerRadius(0).outerRadius(radius + 130);
var colorScale = d3.scaleOrdinal().range(['#98DC4B', 'purple', '#fec44f']);
d3.csv(require('/data/time-breakdown.csv')).then(ready).catch(function (err) {
  return console.log('Failed with', err);
});

function ready(datapoints) {
  console.log(pie(datapoints)); // const names = pie(datapoints).map(d => d.data.task)
  // labelArc.domain(names)

  svg.selectAll('path').data(pie(datapoints)).enter().append('path').attr('d', function (d) {
    return arc(d);
  }).attr('fill', function (d) {
    return colorScale(d.data.task);
  });
  svg.selectAll('.label').data(pie(datapoints)).enter().append('text').text(function (d) {
    return d.data.task;
  }).attr('d', function (d) {
    return labelArc(d);
...
},{"d3":"../node_modules/d3/index.js","/data/time-breakdown.csv":"data/time-breakdown.csv"}]},{},["scripts/homework-13/01-pie.js"], null)
//# sourceMappingURL=/01-pie.9b0eb22e.js.map