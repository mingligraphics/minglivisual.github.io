...
},{}],"scripts/homework-15/01-chart.js":[function(require,module,exports) {
"use strict";
...
function ready(datapoints) {
  ...
  }).attr('fill', 'lightgrey').attr('class', function (d) {
    return d.continent.toLowerCase().replace(/[^a-z]*/g, '');
  });
  svg.append('text').text('higher GDP ⟶').attr('class', 'gdp-note').attr('text-anchor', 'middle').attr('font-size', 12).attr('x', width * 0.75).attr('y', height + 15);
  svg.append('text').text('⟵ lower GDP').attr('class', 'gdp-note').attr('text-anchor', 'middle').attr('font-size', 12).attr('x', width * 0.25).attr('y', height + 15);
  var yAxis = d3.axisLeft(yPositionScale).tickSize(-width).ticks(5).tickFormat(function (d) {
    return d === 80 ? '80 years' : d;
  });
  ...
  function render() {
    ...
    var newHeight = height + margin.top + margin.bottom; // Update your scales - almost always be the same
    ...
    d3.select('.y-axis .domain').remove(); // Update your data points

    svg.selectAll('rect').transition().ease(d3.easeElastic).attr('width', xPositionScale.bandwidth()).attr('x', function (d) {
      return xPositionScale(d.country);
    });
...
},{"d3":"../node_modules/d3/index.js","/data/countries.csv":"data/countries.csv"}]},{},["scripts/homework-15/01-chart.js"], null)
//# sourceMappingURL=/01-chart.ecd53abc.js.map