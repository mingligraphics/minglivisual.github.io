...
},{}],"scripts/homework-17/01-map.js":[function(require,module,exports) {
"use strict";
...
var svg = d3.select('#chart-1').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).style('background', 'black').append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
var colorScale = d3.scaleSequential(d3.interpolateRainbow).domain([0, 1000000]);
...
function ready(_ref) {
  ...
  svg.selectAll('.country').data(countries.features).enter().append('path').attr('class', 'country').attr('d', path).attr('fill', 'black'); // console.log(graticule())

  svg.append('path').datum(graticule()).attr('d', path).attr('stroke', 'grey').lower();
  svg.selectAll('.cities').data(datapoints).enter().append('circle').attr('class', 'cities').attr('r', 1).attr('fill', function (d) {
...
},{"d3":"../node_modules/d3/index.js","topojson":"../node_modules/topojson/index.js","/data/world.topojson":"data/world.topojson","/data/world-cities.csv":"data/world-cities.csv"}]},{},["scripts/homework-17/01-map.js"], null)
//# sourceMappingURL=/01-map.45d67839.js.map