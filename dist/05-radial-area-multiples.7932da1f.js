...
},{}],"scripts/homework-13/05-radial-area-multiples.js":[function(require,module,exports) {
"use strict";
...
var line = d3.radialArea().angle(function (d) {
...
var xPositionScale = d3.scalePoint().range([0, width]).padding(0.25);
...
function ready(datapoints) {
  ...
  var positionNames = datapoints.map(function (d) {
    return d.city;
  });
  ...
  svg.selectAll('g').data(nested).enter().append('g').attr('transform', function (d) {
    return "translate(".concat(xPositionScale(d.key), ",100)");
  }).each(function (d) {
    var name = d.key;
    var datapoints = d.values;
    datapoints.push(datapoints[0]);
    console.log(d); // What svg are we in? Let's grab it

    var g = d3.select(this);
    g.append('path').datum(datapoints).attr('d', line).attr('fill', 'pink').attr('stroke', 'none').attr('opacity', 0.6);
    g.append('circle').attr('r', 3).attr('cx', 0).attr('cy', 0);
    var bands = [40, 60, 80, 100]; // Draw a circle for each item in bands

    g.selectAll('.band').data(bands).enter().append('circle').attr('fill', 'none').attr('stroke', 'grey').attr('r', function (d) {
      console.log(d);
      return radiusScale(d);
    }).lower();
    g.selectAll('.label').data(bands).enter().append('text').text(function (d) {
      return d;
    }).attr('y', function (d) {
      return -radiusScale(d);
    }).attr('text-anchor', 'middle').attr('alignment-baseline', 'middle');
    g.selectAll('.title').data(datapoints).enter().append('text').text(name).attr('transform', function (d) {
      return "translate(".concat(xPositionScale(d.key), ",0)");
    }).attr('dy', 5).attr('dx', 0).attr('text-anchor', 'middle');
  });
}
},{"d3":"../node_modules/d3/index.js","/data/all-temps.csv":"data/all-temps.csv"}]},{},["scripts/homework-13/05-radial-area-multiples.js"], null)
//# sourceMappingURL=/05-radial-area-multiples.7932da1f.js.map