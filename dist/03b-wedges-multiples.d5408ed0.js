...
},{}],"scripts/homework-13/03b-wedges-multiples.js":[function(require,module,exports) {
"use strict";
...
var svg = d3.select('#chart-3b').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
...
var piePositionScale = d3.scalePoint().range([0, width]).padding(0.15);
...
function ready(datapoints) {
  ...
  var maxTemp = d3.max(datapoints, function (d) {
    return d.high_temp;
  });
  var minTemp = d3.min(datapoints, function (d) {
    return d.low_temp;
  });
  ...
  svg.selectAll('g').data(nested).enter().append('g').attr('transform', function (d) {
    return "translate(".concat(piePositionScale(d.key), ",100)");
  }).each(function (d) {
    var name = d.key;
    ...
    g.selectAll('path').data(pie(datapoints)).enter().append('path').attr('d', function (d) {
      return arc(d);
    }).attr('fill', function (d) {
      return colorScale(d.data.high_temp);
    });
    console.log('The new data is', pie(datapoints));
    g.selectAll('.title').data(pie(datapoints)).enter().append('text').text(name).attr('transform', function (d) {
      return "translate(".concat(piePositionScale(d.key), ",0)");
    }).attr('dy', 120).attr('dx', -20);
  });
}
},{"d3":"../node_modules/d3/index.js","/data/all-temps.csv":"data/all-temps.csv"}]},{},["scripts/homework-13/03b-wedges-multiples.js"], null)
//# sourceMappingURL=/03b-wedges-multiples.d5408ed0.js.map