...
},{}],"scripts/homework-17/02-map.js":[function(require,module,exports) {
"use strict";
...
var projection = d3.geoEqualEarth(); // out geoPath needs a PROJECTION variable

var path = d3.geoPath().projection(projection);
Promise.all([d3.json(require('/data/world.topojson')), d3.csv(require('/data/airport-codes-subset.csv'))]).then(ready).catch(function (err) {
...
function ready(_ref) {
  ...
  svg.append('path').datum({
    type: 'Sphere'
  }).attr('d', path).attr('stroke', 'black').attr('fill', '#81DAF5');
  svg.selectAll('.country').data(countries.features).enter().append('path').attr('class', 'country').attr('d', path).attr('fill', 'lightgrey').attr('stroke', 'black');
  svg.selectAll('.airports').data(datapoints).enter().append('circle').attr('class', 'cities').attr('r', 2).attr('fill', 'white').attr('transform', function (d) {
    var coords = projection([d.longitude, d.latitude]);
    return "translate(".concat(coords, ")");
  }); //-----------------------------------------------------------------  
  // Build a new place to keep all the vote data

  var airportStore = d3.map(); //Copy our datapoints into a dictionary.

  datapoints.forEach(function (d) {
    airportStore.set(d.iata_code, [d.longitude, d.latitude]); // otherAirport = airportStore.get(d.iata_code)
    ...
    console.log(geoLine);
    svg.selectAll('path').enter().append('path').data(geoLine).attr('stroke', 'white').attr('fill', 'white').attr('d', path);
  }); //We started off with
...
},{"d3":"../node_modules/d3/index.js","topojson":"../node_modules/topojson/index.js","/data/world.topojson":"data/world.topojson","/data/airport-codes-subset.csv":"data/airport-codes-subset.csv"}]},{},["scripts/homework-17/02-map.js"], null)
//# sourceMappingURL=/02-map.70821644.js.map