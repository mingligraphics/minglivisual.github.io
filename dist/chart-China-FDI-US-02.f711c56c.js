...
var width = 170 - margin.left - margin.right; // I'll give you this part!

var container = d3.select('#chart-China-FDI-US-02'); // Set up margin/height/width
...
var yPositionScale = d3.scaleLinear().domain([0, 100000]).range([height, 0]);
...
Promise.all([d3.csv(require('../data/China-FDI-US-02.csv')), d3.csv(require('../data/China-FDI-Global.csv'))]).then(ready).catch(function (err) {
  console.log(err);
});
...
function ready(_ref) {
  ...
  var names = dataUS.map(function (d) {
    return d.year;
  });
  ...
  container.selectAll('svg').data(nested).enter().append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').each(function (d) {
    ...
    svg.append('path').datum(dataUS).attr('fill', 'none').attr('stroke', 'grey').attr('opacity', 0.8).attr('d', line).style('stroke-width', '2px');
    svg.append('path').datum(datapoints).attr('fill', 'none').attr('stroke', '#9e4b6c').attr('opacity', 0.8).attr('d', line).style('stroke-width', '2px');
    svg.append('text').text('USA').attr('x', 3 / 4 * width).attr('y', 3 / 4 * height).attr('font-size', '12px').attr('dx', -10) // offset 5 pixels to the right
    .attr('dy', -20).attr('alignment-baseline', 'middle').attr('fill', 'grey');
    svg.append('text').text(name).attr('x', width / 2).attr('text-anchor', 'middle') // text aligned
    .attr('dy', -15).style('font-size', '14px').attr('fill', '#9e4b6c').attr('font-weight', 'bold');
    var xAxis = d3.axisBottom(xPositionScale).tickValues([2015, 16, 17, 18]).tickFormat(d3.format('d')).tickSize(-height);
    svg.append('g').attr('class', 'axis x-axis').attr('transform', 'translate(0,' + height + ')').call(xAxis).style('font-size', '12px').lower();
    var yAxis = d3.axisLeft(yPositionScale).tickValues([20000, 40000, 60000, 80000, 100000]).tickSize(-width);
    svg.append('g').attr('class', 'axis y-axis').call(yAxis).style('font-size', '12px').lower();
    svg.selectAll('.tick line').attr('stroke-dasharray', 2).attr('stroke', 'lightgrey');
...
},{"d3":"../node_modules/d3/index.js","../data/China-FDI-US-02.csv":"data/China-FDI-US-02.csv","../data/China-FDI-Global.csv":"data/China-FDI-Global.csv"}]},{},["scripts/chart-China-FDI-US-02.js"], null)
//# sourceMappingURL=/chart-China-FDI-US-02.f711c56c.js.map