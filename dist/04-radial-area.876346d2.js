...
},{}],"scripts/homework-13/04-radial-area.js":[function(require,module,exports) {
"use strict";
...
var svg = d3.select('#chart-4').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').attr('transform', "translate(".concat(width / 2, ",").concat(height / 2, ")"));
var angleScale = d3.scaleBand().range([0, Math.PI * 2]);
var radius = 150;
var radiusScale = d3.scaleLinear().domain([0, 85]).range([0, radius]);
...
function ready(datapoints) {
  ...
  svg.append('path').datum(datapoints).attr('d', line).attr('fill', 'lightblue').attr('stroke', 'none').attr('opacity', 0.6);
  svg.append('circle').attr('r', 3).attr('cx', 0).attr('cy', 0);
  var bands = [30, 40, 50, 60, 70, 80, 90]; // Draw a circle for each item in bands

  svg.selectAll('.band').data(bands).enter().append('circle').attr('fill', 'none').attr('stroke', 'grey').attr('r', function (d) {
    console.log(d);
  ...
  svg.append('text').text('NYC').attr('fill', 'black').attr('stroke', 'none').attr('font-weight', '500').style('font-size', '26px').attr('transform', 'translate(-25,10)');
}
},{"d3":"../node_modules/d3/index.js","/data/ny-temps.csv":"data/ny-temps.csv"}]},{},["scripts/homework-13/04-radial-area.js"], null)
//# sourceMappingURL=/04-radial-area.876346d2.js.map