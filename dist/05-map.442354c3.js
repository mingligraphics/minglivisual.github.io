...
},{}],"scripts/homework-17/05-map.js":[function(require,module,exports) {
"use strict";
...
var radiusScale = d3.scaleSqrt().range([1, 6]); // const colorScale = d3.scaleOrdinal().range(['#a6cee3',
...
function ready(_ref) {
  ...
  radiusScale.domain([0, max]); // const Sources = datapoints.PrimSource
  ...
  svg.selectAll('.state').data(states.features).enter().append('path').attr('class', 'state').attr('d', path).attr('fill', 'lightgrey'); // Add a text element for every single state
  ...
  }).attr('stroke', 'black').attr('fill', 'none'); // Make a circle for every single wafflehouse
  // ps they're in datapoints
  // you don't have to position them
  // radius of.... 2?

  svg.selectAll('.powerplants').data(datapoints).enter().append('circle').attr('class', 'powerplants').attr('r', function (d) {
    // console.log(d)
    return radiusScale(d.Total_MW);
  }).attr('fill', 'red').attr('transform', function (d) {
    var coords = projection([d.Longitude, d.Latitude]);
    return "translate(".concat(coords, ")");
  }).attr('opacity', 0.6); // .attr('cx', d => {
  //   console.log(d)
...
},{"d3":"../node_modules/d3/index.js","topojson":"../node_modules/topojson/index.js","/data/us_states.topojson":"data/us_states.topojson","/data/powerplants.csv":"data/powerplants.csv"}]},{},["scripts/homework-17/05-map.js"], null)
//# sourceMappingURL=/05-map.442354c3.js.map