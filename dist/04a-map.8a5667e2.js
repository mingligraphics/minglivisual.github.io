...
},{}],"scripts/homework-17/04a-map.js":[function(require,module,exports) {
"use strict";
...
var svg = d3.select('#chart-4a').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
var colorScale = d3.scaleOrdinal().domain(['Romney', 'Obama']).range(['purple', 'green']);
var opacityScale = d3.scaleLinear().domain([50, 100]).range([0, 1]);
...
function ready(json) {
  ...
  var counties = topojson.feature(json, json.objects.elpo12p010g); // Not sure how to do scale/center/etc?
  // Just use .fitSize to center your map
  // and set everything up nice

  projection.fitSize([width, height], counties);
  svg.selectAll('.counties').data(counties.features).enter().append('path').attr('class', 'counties').attr('d', path).attr('stroke', 'none').attr('fill', function (d) {
    console.log(d);
...
},{"d3":"../node_modules/d3/index.js","topojson":"../node_modules/topojson/index.js","/data/counties.topojson":"data/counties.topojson"}]},{},["scripts/homework-17/04a-map.js"], null)
//# sourceMappingURL=/04a-map.8a5667e2.js.map