...
},{}],"scripts/homework-15/03-chart.js":[function(require,module,exports) {
"use strict";
...
var radius = 200;
var radiusScale = d3.scaleLinear().domain([10, 100]).range([40, radius]);
var angleScale = d3.scalePoint().domain(['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sept', 'Oct', 'Nov', 'Dec', 'Blah']).range([0, Math.PI * 2]);
var line = d3.radialArea().outerRadius(function (d) {
...
function ready(datapoints) {
  ...
  container.selectAll('.bands').data(circleBands).enter().append('circle').attr('class', 'bands').attr('fill', 'none').attr('stroke', 'gray').attr('cx', 0).attr('cy', 0).attr('r', function (d) {
    return radiusScale(d);
  }).lower();
  container.append('text').attr('text-anchor', 'middle').attr('class', 'city-name').text('NYC').attr('font-size', 30).attr('font-weight', 700).attr('alignment-baseline', 'middle');
  container.selectAll('.temp-notes').data(textBands).enter().append('text').attr('class', 'temp-notes').attr('x', 0).attr('y', function (d) {
    return -radiusScale(d);
  }).attr('dy', -2).text(function (d) {
    return d + '°';
  ...
  d3.select('#nyc-step').on('stepin', function () {
    svg.selectAll('.nyc-temp').attr('fill', 'green').style('visibility', 'visible');
    svg.selectAll('.tuscon-temp').style('visibility', 'hidden');
    svg.selectAll('.bj-temp').style('visibility', 'hidden');
    svg.selectAll('.lima-temp').style('visibility', 'hidden');
    svg.selectAll('.stockholm-temp').style('visibility', 'hidden');
  ...
  d3.select('#beijing-step').on('stepin', function () {
    svg.selectAll('.bj-temp').attr('fill', 'blue').style('visibility', 'visible');
    svg.selectAll('.nyc-temp').style('visibility', 'hidden');
    svg.selectAll('.tuscon-temp').style('visibility', 'hidden');
    svg.selectAll('.lima-temp').style('visibility', 'hidden');
    svg.selectAll('.stockholm-temp').style('visibility', 'hidden');
  ...
  d3.select('#stockholm-step').on('stepin', function () {
    svg.selectAll('.stockholm-temp').attr('fill', 'purple').style('visibility', 'visible');
    svg.selectAll('.bj-temp').style('visibility', 'hidden');
    svg.selectAll('.nyc-temp').style('visibility', 'hidden');
    svg.selectAll('.tuscon-temp').style('visibility', 'hidden');
    svg.selectAll('.lima-temp').style('visibility', 'hidden');
  ...
  d3.select('#lima-step').on('stepin', function () {
    svg.selectAll('.lima-temp').attr('fill', 'orange').style('visibility', 'visible');
    svg.selectAll('.stockholm-temp').style('visibility', 'hidden');
    svg.selectAll('.bj-temp').style('visibility', 'hidden');
    svg.selectAll('.nyc-temp').style('visibility', 'hidden');
    svg.selectAll('.tuscon-temp').style('visibility', 'hidden');
  ...
  d3.select('#tuscon-step').on('stepin', function () {
    svg.selectAll('.tuscon-temp').attr('fill', 'red').style('visibility', 'visible');
    svg.selectAll('.lima-temp').style('visibility', 'hidden');
    svg.selectAll('.stockholm-temp').style('visibility', 'hidden');
    svg.selectAll('.bj-temp').style('visibility', 'hidden');
    svg.selectAll('.nyc-temp').style('visibility', 'hidden');
  ...
  var nycDatapoints = datapoints.filter(function (d) {
    return d.city === 'NYC';
  });
  nycDatapoints.push(nycDatapoints[0]);
  container.append('path').attr('class', 'nyc-temp').datum(nycDatapoints).attr('d', line).attr('fill', 'none').attr('opacity', 0.75);
  var bjDatapoints = datapoints.filter(function (d) {
    return d.city === 'Beijing';
  });
  bjDatapoints.push(bjDatapoints[0]);
  container.append('path').attr('class', 'bj-temp').datum(bjDatapoints).attr('d', line).attr('fill', 'none').attr('opacity', 0.75);
  var limaDatapoints = datapoints.filter(function (d) {
    return d.city === 'Lima';
  });
  limaDatapoints.push(limaDatapoints[0]);
  container.append('path').attr('class', 'lima-temp').datum(limaDatapoints).attr('d', line).attr('fill', 'none').attr('opacity', 0.75);
  var tusconDatapoints = datapoints.filter(function (d) {
    return d.city === 'Tuscon';
  });
  tusconDatapoints.push(tusconDatapoints[0]);
  container.append('path').attr('class', 'tuscon-temp').datum(tusconDatapoints).attr('d', line).attr('fill', 'none').attr('opacity', 0.75);
  var stockholmDatapoints = datapoints.filter(function (d) {
    return d.city === 'Stockholm';
  });
  stockholmDatapoints.push(stockholmDatapoints[0]);
  container.append('path').attr('class', 'stockholm-temp').datum(stockholmDatapoints).attr('d', line).attr('fill', 'none').attr('opacity', 0.75);
  /* 
   Responsive design render function
   */

  function render() {
    console.log('Rendering'); // This section will always be the same (except the last line, maybe)
    ...
    actualSvg.attr('width', svgWidth).attr('height', svgHeight); // always be the same

    var newWidth = svgWidth - margin.left - margin.right;
    var newHeight = svgHeight - margin.top - margin.bottom; // Update your scales - almost always be the same

    radiusScale.range([0, Math.min((newWidth - 60) / 2, (newHeight - 60) / 2)]);
    line.outerRadius(function (d) {
      return radiusScale(d.high_temp);
    }).innerRadius(function (d) {
      return radiusScale(d.low_temp);
    ...
    container.selectAll('.temp-notes').attr('y', function (d) {
      return -radiusScale(d);
...
},{"d3":"../node_modules/d3/index.js","/data/all-temps.csv":"data/all-temps.csv"}]},{},["scripts/homework-15/03-chart.js"], null)
//# sourceMappingURL=/03-chart.f74e6a69.js.map