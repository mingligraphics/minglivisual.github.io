...
},{}],"scripts/homework-15/02-chart.js":[function(require,module,exports) {
"use strict";
...
var colorScale = d3.scaleOrdinal().range(['#8dd3c7', '#ffffb3', '#bebada', '#fb8072', '#80b1d3', '#fdb462', '#b3de69', '#fccde5', '#d9d9d9', '#bc80bd']);
...
function ready(datapoints) {
  ...
  d3.select('#regions-step').on('stepin', function () {
    svg.selectAll('path').attr('stroke', function (d) {
      ...
      if (d.key === 'Mountain' || d.key === 'Pacific' || d.key === 'West South Central' || d.key === 'South Atlantic') {
        return 'blue';
      } else {
        return 'lightgrey';
      }
  ...
  svg.selectAll('path').data(nested).enter().append('path').attr('class', 'region-path').attr('d', function (d) {
    return line(d.values);
  }).attr('stroke', function (d) {
    return colorScale(d.key);
  }).attr('stroke-width', 2).attr('fill', 'none');
  svg.selectAll('circle').data(nested).enter().append('circle').attr('class', 'region-circle').attr('fill', function (d) {
    return colorScale(d.key);
  }).attr('r', 4).attr('cy', function (d) {
    return yPositionScale(d.values[0].price);
  }).attr('cx', function (d) {
    return xPositionScale(d.values[0].datetime);
  });
  svg.selectAll('text').data(nested).enter().append('text').attr('class', 'region-text').attr('y', function (d) {
    return yPositionScale(d.values[0].price);
  }).attr('x', function (d) {
    return xPositionScale(d.values[0].datetime);
  }).text(function (d) {
    return d.key;
  }).attr('dx', 6).attr('dy', 4).attr('font-size', '12');
  svg.append('text').attr('font-size', '24').attr('text-anchor', 'middle').text('U.S. housing prices fall in winter').attr('class', 'title-text').attr('x', width / 2).attr('y', -40).attr('dx', 40);
  var rectWidth = xPositionScale(parseTime('February-17')) - xPositionScale(parseTime('November-16'));
  svg.append('rect').attr('class', 'winter-rect').attr('x', xPositionScale(parseTime('December-16'))).attr('y', 0).attr('width', rectWidth).attr('height', height).attr('fill', '#C2DFFF').lower();
  ...
  function render() {
    ...
    yPositionScale.range([newHeight, 0]); // Maybe have fewer axis ticks if the window is smaller
    ...
    svg.select('.x-axis').transition().attr('transform', 'translate(0,' + newHeight + ')').call(xAxis); // Update your data points

    svg.selectAll('.region-circle').transition().ease(d3.easeElastic).attr('cy', function (d) {
      return yPositionScale(d.values[0].price);
    }).attr('cx', function (d) {
      return xPositionScale(d.values[0].datetime);
    });
    svg.selectAll('.region-text').transition().ease(d3.easeElastic).attr('y', function (d) {
      return yPositionScale(d.values[0].price);
    }).attr('x', function (d) {
      return xPositionScale(d.values[0].datetime);
    });
    svg.selectAll('.region-title').transition().ease(d3.easeElastic).attr('x', newWidth / 2);
    var rectWidth = xPositionScale(parseTime('February-17')) - xPositionScale(parseTime('November-16'));
    svg.selectAll('.winter-rect').attr('x', xPositionScale(parseTime('December-16'))).attr('width', rectWidth).attr('height', newHeight);
...
},{"d3":"../node_modules/d3/index.js","/data/housing-prices.csv":"data/housing-prices.csv"}]},{},["scripts/homework-15/02-chart.js"], null)
//# sourceMappingURL=/02-chart.293cb068.js.map